...
def mass_dashboards(access_token,server_id,server_domain):
    ...
    try:
        dash_extraction_dir = "dashboard_backup"
        os.mkdir(dash_extraction_dir)
    ...
    try:
        dash_extraction_dir = 'mass_dashboard_backup_{}'.format(dt_string)
        os.mkdir(dash_extraction_dir)
    except OSError as error:
            pass
    ...
    if os_ == "Windows":
        d_ext = "{}".format(cd)+"\\{}\\".format(dash_extraction_dir)
    else:
        d_ext = "{}".format(cd)+"/{}/".format(dash_extraction_dir)
    ...
    os.chdir("..")
    directory = './{}'.format(dash_extraction_dir)
    tcrm_zipper(directory,dash_extraction_dir)
    shutil.rmtree(r'./{}'.format(dash_extraction_dir))
