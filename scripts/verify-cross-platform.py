...
def check_sf_cli():
    ...
    try:
        ...
        if result.returncode == 0:
            print(f"[OK] SF CLI: {result.stdout.strip()}")
        else:
            print("[WARN] SF CLI not found (install from https://developer.salesforce.com/tools/sfdxcli)")
    except FileNotFoundError:
        print("[WARN] SF CLI not found (install from https://developer.salesforce.com/tools/sfdxcli)")
    except Exception as e:
        print(f"[WARN] SF CLI check failed: {e}")
...
def check_platform_utils():
    ...
    os_type = get_os()
    print(f"[OK] OS detected: {os_type}")
    print(f"[OK] Config dir: {get_config_dir()}")
    print(f"[OK] Data dir: {get_data_dir()}")
